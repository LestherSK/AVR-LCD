...
char line2[LCD_WIDTH];	//holds value of second line
//char line1Full;					
...
void lcd_put(char i) {
	LCD &= 0x0F; 	     //Clear upper four bits
	LCD |= (i&0xF0);   //put upper four bits of data on output Port
}
/**********************************************************/
void lcd_toggleClock(void) {
	sbi(LCD, LCD_E);   //LCD_E = 1;
	_delay_ms(1); 		   //enable pulse width  >= 300ns
	cbi(LCD, LCD_E);   //LCD_E = 0;  		//Clock enable: falling edge 
	_delay_ms(1);
}
/**********************************************************/
void lcd_command(char i) {
	lcd_put(i);						//put data on output Port
	cbi(LCD, LCD_RS);			//LCD_RS = 0;			//D/I=LOW : send instruction 
	cbi(LCD, LCD_RW);			//LCD_RW = 0;			 //R/W=LOW : Write      
...
void lcd_write(char i) {
	lcd_put(i);						//put data on output Port
	sbi(LCD, LCD_RS);			//LCD_RS =1;			//D/I=HIGH : send data 
	cbi(LCD, LCD_RW);			//LCD_RW =0;			//R/W=LOW : Write     
...
void lcd_init() {
	...
	*/
	...
	lcd_toggleClock();
	lcd_put(0);						//Clear output port
	_delay_ms(15);				//Wait >15 msec after power is applied 
...
unsigned char lcd_busy_wait(void)
{
	...
	// cbi(LCD, LCD_RS); //LCD_RS = 0;
	// sbi(LCD, LCD_RW); //LCD_RW = 1;
	// LCD_TRIS |= 0xF0; //set to input
	// while(!(LCD&0x80));		//wait for the busy flag to become false
	// //value = LCD;
